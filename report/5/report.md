...
　まず文書1--6に含まれる単語それぞれの単語頻度`tf`を文書ごとに、
データフレーム`df`に与える[^R]。
...
類似度を求めるために各文書から2つ選択する組み合わせを
`docs.C.2`リストとして作成する。
先ほどの`df`の列名は文書名を示すので、
`colnames`関数で`df`のカラム名を取得し、
それらから2つを選択する組み合わせを`combn`関数で取得する。
そして取得した組み合わせのリストを
`t`関数で転値行列をつくりデータフレームとして
`docs.C.2`に与える。
